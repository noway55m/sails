(function($) {
	...
	$.fn.draw = function(o,obj,selectedpage){
		...
		_first.attr('ng-click', 'loadPage($event, 1)');	
		...
		for(var i = 0; i < o.count; i++){
			...
			if(val == selectedpage){
				...
				_obj = $(document.createElement('li')).html('<a href="#">'+ val +'</a>');
			}
			_ul.append(_obj);
			_obj.attr('ng-click', 'loadPage($event' + ',' + val + ')');
		}		
		...
		_last.attr('ng-click', 'loadPage($event' + ',' + o.count + ')');		
			...
			_rotright.hover(
				function() {
				  thumbs_scroll_interval = setInterval(
					function() {
					  var left22 = _ulwrapdiv.scrollLeft() + 3;
					  _ulwrapdiv.scrollLeft(left22);
					},
					20
				  );
				},
		...
		_ulwrapdiv.find('li').click(function(e){
			...
			if(ver == 'ie7')
				...
				_ulwrapdiv.animate({scrollLeft: left + tmp - 33 - _first.parent().width() + 'px'});	
			o.onChange(currval);	
		});
		...
		else _ulwrapdiv.animate({scrollLeft: left + tmp - 33 - _first.parent().width() + 'px'});	
	}
